...
def schedule(request):
  now = datetime.datetime.now()
  #now = datetime.datetime.now() + datetime.timedelta(days=200)
  meets = Meet.objects.select_related().filter(occurred_at__year=now.year).order_by('occurred_at')

  next_meet = None
  for meet in meets:
    if not meet.past_tense():
      meet.next_meet = True
      next_meet = meet
      break
  for meet in meets:
    meet.races = meet.race_set.order_by('time')
    if len(meet.races) > 0:
      meet.have_race_times = True
      for race in meet.races:
        if not race.time:
          meet.have_race_times = False
    else:
      meet.have_race_times = False


  return render(request, 'schedule/schedule.html', { 'meets': meets, 'now': now, 'next_meet': next_meet, },)
